extern Txt K_20;
extern Txt K_20_2Dp;
extern Txt K_20a_20_2Dy_20;
extern Txt Kutf_2D8;
Asm4d_Proc proc_LEP__ESCAPE;
Asm4d_Proc proc_LEP__ESCAPE__PATH;
Asm4d_Proc proc_Z7__EXECUTE;
extern unsigned char D_proc_Z7__ARCHIVE[];
void proc_Z7__ARCHIVE( Asm4d_globals *glob, tProcessGlobals *ctx, int32_t inNbExplicitParam, int32_t inNbParam, PCV inParams[], CV *outResult)
{
	CallChain c(ctx,D_proc_Z7__ARCHIVE);
	if (!ctx->doingAbort) try {
		...
		ltargetPath=Parm<Txt>(inParams,inNbParam,2).get();
		{
			...
			Txt t4;
			g->AddString(K_20a_20_2Dy_20.get(),t3.get(),t4.get());
			Txt t5;
			g->AddString(t4.get(),K_20.get(),t5.get());
			Txt t6;
			t6=lsourcePath.get();
			Txt t7;
